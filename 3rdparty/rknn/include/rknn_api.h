...
/*
    the mode of running on target NPU core. 枚举类型
*/
...
*/
// rknn_tensor_attr 结构体在 YOLOv5 结合 RKNN 推理框架的场景中
// 为准确描述和配置输入输出张量的属性提供了全面的信息，确保模型能够正确地进行推理
// attribute: 属性
typedef struct _rknn_tensor_attr {
    ...
    uint32_t n_dims;                                    /* the number of dimensions. */ //张量的维度数量
    uint32_t dims[RKNN_MAX_DIMS];                       /* the dimensions array. */ // 存储张量各个维度的大小
    char name[RKNN_MAX_NAME_LEN];                       /* the name of tensor. */ 

    uint32_t n_elems;                                   /* the number of elements. */ // 张量中元素的数量
    uint32_t size;                                      /* the bytes size of tensor. */ // 张量占用的字节大小

    rknn_tensor_format fmt;                             /* the data format of tensor. */ // 张量的数据格式
    rknn_tensor_type type;                              /* the data type of tensor. */ // 张量的数据类型
    rknn_tensor_qnt_type qnt_type;                      /* the quantitative type of tensor. */ // 张量的量化类型
    int8_t fl;                                          /* fractional length for RKNN_TENSOR_QNT_DFP. */ // 小数长度
    int32_t zp;                                         /* zero point for RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC. */ // 表示零点
    float scale;                                        /* scale for RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC. */ // 表示缩放因子。
...
typedef struct _rknn_input {
    ...
    uint8_t pass_through;                               /* pass through mode.
                                                           ...
                                                           if FALSE, the buf data is converted into an input consistent with the model
                                                                     according to the following type and fmt. so the following variables
                                                                     need to be set.*/
    // 输入缓冲区（buf）中的数据需要根据后续设置的 type 和 fmt 进行转换，以确保数据格式与模型的输入要求一致
    // 为True时为直通模式：输入缓冲区（buf）中的数据会直接传递给 RKNN 模型的输入节点，不会进行任何格式转换或类型转换
    // 由于不需要进行转换，因此后续与数据类型（rknn_tensor_type type）和数据格式（rknn_tensor_format fmt）相关的变量不需要进行设置。
    // 为false为非直通模式：输入缓冲区（buf）中的数据需要根据后续设置的 type 和 fmt 进行转换，以确保数据格式与模型的输入要求一致。
    // 因此，需要正确设置 rknn_tensor_type type 和 rknn_tensor_format fmt 这两个变量
